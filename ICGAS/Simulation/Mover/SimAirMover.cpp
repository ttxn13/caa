...
E_MOVER SimAirMover::mover_type() const
{
	return E_MOVER::AIR_MOVER;
}

void SimAirMover::step(SimPlatform* platform, S_ROUTE_POINT* route_point, double step_s)
{
	// 创建变量，存储平台状态和能力
	S_PLAT_STATE* plat_state = &platform->plat_state;
	const S_AIR_MOVER_CAP* cap = std::get_if<S_AIR_MOVER_CAP>(&platform->cap);
	...
	target_point.pos_lla.alt_m = std::clamp(target_point.pos_lla.alt_m, cap->min_alt_m, cap->max_alt_m);
	...
	target_point.speed_ms > plat_state->vel_loc.speed_ms ?
		plat_state->vel_loc.speed_ms = std::min(target_point.speed_ms, plat_state->vel_loc.speed_ms + cap->max_linear_load_g * GRAVITY_MS2 * step_s) :
		plat_state->vel_loc.speed_ms = std::max(target_point.speed_ms, plat_state->vel_loc.speed_ms - cap->max_linear_load_g * GRAVITY_MS2 * step_s);
	
	// 计算目标高度
	target_point.pos_lla.alt_m > plat_state->pos_lla.alt_m ?
		plat_state->pos_lla.alt_m = std::min(target_point.pos_lla.alt_m, plat_state->pos_lla.alt_m + cap->max_climb_vel_ms * GRAVITY_MS2 * step_s) :
		plat_state->pos_lla.alt_m = std::max(target_point.pos_lla.alt_m, plat_state->pos_lla.alt_m - cap->max_drop_vel_ms  * GRAVITY_MS2 * step_s);
	target_point.pos_lla.alt_m > plat_state->pos_lla.alt_m ?
		plat_state->vel_loc.path_ang_rad = std::atan(( cap->max_climb_vel_ms * GRAVITY_MS2) * step_s / plat_state->vel_loc.speed_ms) :
		plat_state->vel_loc.path_ang_rad = std::atan((-cap->max_drop_vel_ms  * GRAVITY_MS2) * step_s / plat_state->vel_loc.speed_ms) ;
	...
	const double omega_rads = cap->max_radial_load_g * GRAVITY_MS2 / plat_state->vel_loc.speed_ms;
	...
	if (omega_rads * step_s < fabs(del_track_ang_rad)) {
		(del_track_ang_rad > 0) ?
			[&] { S_PLAT_STATE del = step_del_radial(plat_state, omega_rads * step_s, cap->max_radial_load_g); add_plat_state(plat_state, &del, step_s); }() :
			[&] { S_PLAT_STATE del = step_del_radial(plat_state, -omega_rads * step_s, cap->max_radial_load_g); add_plat_state(plat_state, &del, step_s); }();
		return;
	}

	// 转向段+直线段迭代
	double step_radial_s = fabs(del_track_ang_rad) / omega_rads;
	double step_linear_s = step_s - step_radial_s;
	[&] { S_PLAT_STATE del = step_del_radial(plat_state, del_track_ang_rad, cap->max_radial_load_g); add_plat_state(plat_state, &del, step_radial_s); }();
	[&] { S_PLAT_STATE del = step_del_linear(plat_state); add_plat_state(plat_state, &del, step_linear_s); }();
}

void SimAirMover::add_plat_state(S_PLAT_STATE* plat_state, const S_PLAT_STATE* del_plat_state, double step_s)
{
	...
	plat_state->vel_loc.path_ang_rad	+= del_plat_state->vel_loc.path_ang_rad * step_s;
	plat_state->vel_loc.track_ang_rad	 = UtilCoor::norm_rad_0_2pi(plat_state->vel_loc.track_ang_rad);
	plat_state->vel_loc.path_ang_rad	 = UtilCoor::norm_rad_pi_pi(plat_state->vel_loc.path_ang_rad);
}

S_PLAT_STATE SimAirMover::step_del_linear(const S_PLAT_STATE* plat_state) const
{
...
S_PLAT_STATE SimAirMover::step_del_radial(const S_PLAT_STATE* plat_state, const double del_angle_rad, const double max_radial_load_g) const
{
	S_PLAT_STATE del_plat_state;

	const double omega_rads = max_radial_load_g * GRAVITY_MS2 / plat_state->vel_loc.speed_ms;
	...
	const double turn_radius_m = plat_state->vel_loc.speed_ms * plat_state->vel_loc.speed_ms /
